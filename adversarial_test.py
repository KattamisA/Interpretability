...
for i in range(len(image_dataset)):
    ...
    _, ranks = classification(orig, sort=True, show=False, cuda=False)
    ...
    for eps in range(1, 101):
        ...
        adv, _, _ = adversarial_examples("data/{}".format(image_path), method="SSLLCI", eps=eps, show=False, cuda=False)

        confs, _ = classification(adv, sort=False, show=False, cuda=False)
        orig_conf = confs[0, original_class]
        ll_conf = confs[0, ll_class]
        q = open("results/adversarial_examples/Adversarial_test/ssllci/{}.txt".format(image_name), "a")
